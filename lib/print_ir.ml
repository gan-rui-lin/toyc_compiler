...
let string_of_ir_inst = function
  | Binop (op, dst, lhs, rhs) ->
      Printf.sprintf "%s = %s %s %s" (string_of_operand dst) (string_of_operand lhs) op (string_of_operand rhs)
  | Unop (op, dst, src) ->
      Printf.sprintf "%s = %s%s" (string_of_operand dst) op (string_of_operand src)
  | Load (dst, src) ->
  ...
  | Call (ret, fname, args) ->
      Printf.sprintf "%s = call %s(%s)" (string_of_operand ret) fname (string_of_operands args)
  | Ret None ->
      "return"
  | Ret (Some op) ->
