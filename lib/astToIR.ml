...
let rec expr_to_ir (ctx: context) (e: expr) : operand * ir_inst list =
  ...
  | Binop (Land, e1, e2) ->
      ...
      let code =
        ...
        @ [IfGoto (lhs, l_false)]
        (* 2. 否则计算 rhs *)
        @ (let rhs, c2 = expr_to_ir ctx e2 in c2 @ [Assign (res, rhs)])
        (* 3. 跳到结束 *)
  ...
  | Call (f, args) ->
      (* 参数顺序按出现顺序计算 *)
      let codes, ops =
        List.fold_left (fun (acc_c, acc_o) arg ->
...
let rec stmt_to_res (ctx: context) (s: stmt) : stmt_res =
  ...
  | Empty -> Normal []
  | ExprStmt e ->
      let _, code = expr_to_ir ctx e in Normal code
  ...
  | While (cond, body) ->
      ...
      let code =
        ...
        @ ccode @ [IfGoto (cnd, lbody); Goto lend]
